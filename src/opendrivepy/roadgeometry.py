...
import sys
sys.path.append("/home/byq/byq/code/geely/xodr-OSM-Converter/src/")
...
class RoadParamPoly3(RoadGeometry):
    ...
    def __init__(
        self, s, x, y, hdg, length, aU, bU, cU, dU, aV, bV, cV, dV, pRange
    ):
        super(RoadParamPoly3, self).__init__(s, x, y, hdg, length, 'paramPoly3')
    ...
    def generate_coords(self):
        ...
        for n in array:
            # Position
            pos = (n/ self.length) * self._pRange
            ...
            xrot = x * np.cos(self.hdg) - y * np.sin(self.hdg)
            yrot = x * np.sin(self.hdg) + y * np.cos(self.hdg)
            ...
            self.points.append(Point((self.x+xrot),(self.y+yrot),(self.s+n),(self.hdg+tangent)))